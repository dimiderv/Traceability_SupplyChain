...
# installChaincode PEER1 ORG2
installChaincodePeer1Org2() {
  ORG=$1
  setGlobals $ORG
...
# queryInstalled PEER1 ORG1
queryInstalledPeer1Org1() {
  ORG=$1
  setGlobals $ORG
  export CORE_PEER_ADDRESS=localhost:8051
  set -x
...
# queryInstalled PEER1 ORG2
queryInstalledPeer1Org2() {
  ORG=$1
  setGlobals $ORG
  export CORE_PEER_ADDRESS=localhost:10051
  set -x
...
# query channel info for every peer
queryChannelInfo() {
  ORG=$1
  setGlobals $ORG
  
  set -x
  peer channel getinfo -c mychannel >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
  verifyResult $res "Query channel info peer0.org${ORG} has failed"
  successln "Query installed successful on peer0.org${ORG} on channel"
  ...
  { set +x; } 2>/dev/null
  cat log.txt
  verifyResult $res "Query channel info peer1.org${ORG} has failed"
  successln "Query installed successful on peer1.org${ORG} on channel"
...
installChaincodePeer1Org1 1
infoln "Install chaincode on peer0.org2..."
installChaincode 2
infoln "Install chaincode on peer0.org2..."
installChaincodePeer1Org2 2
infoln "Install chaincode on peer0.org3..."
installChaincode 3

infoln " query whether the chaincode is installed on peer0 org1 "
queryInstalled 1
infoln " query whether the chaincode is installed on peer1 org1 "
queryInstalledPeer1Org1 1
...
infoln " Check if All peers have the correct blockchain"
queryChannelInfo 1 2 3 
